...
def get_response(fiat, asset, bank):
    """
    fiat-> { "UAH" }
...
def exchange_rate(your_list ,request_text, to_start=None):
    
    word = request_text.find('price":"', to_start) + 8    #indent to price
    result_word=""  #to add characters
    
    if len(result) < 10 :   #how many offers
        for i in range(100):
            if request_text[word+i] != "\"":   #example: "133.23 '"'<- to last char
                result_word += request_text[word+i]
            else:
                your_list.append(result_word)  #add in "your_list"
                break
        exchange_rate(result, request_text, word+i)   #recursive from an index that is checked
    else:
        return result


if __name__=="__main__":
    response = get_response("UAH", "USDT", "Monobank").text
    
    result = []

    exchange_rate(result, response)
